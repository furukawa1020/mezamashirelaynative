            ...
            <div key={m.id} style={{ padding: 8, borderBottom: '1px solid #eee' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="button" onClick={() => addStep(m.id)}>ステップ追加</button>
                  <button className="button" style={{ background: '#ff4444', color: 'white' }} onClick={() => remove(m.id)}>削除</button>
                </div>
              </div>
              <div style={{ fontSize: 12, color: '#666' }}>起床時間: {m.wake_time}</div>
              <div style={{ marginTop: 8 }}>
                {(m.steps || []).map((s: any) => (
                  <div key={s.id} style={{ padding: 6, borderTop: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between' }}>
                    <span>{s.order}. {s.label} <span style={{ fontSize: 12, color: '#666' }}>({s.type})</span></span>
                    <button style={{ border: 'none', background: 'none', color: '#ff4444', cursor: 'pointer' }} onClick={() => removeStep(m.id, s.id)}>×</button>
                  </div>
                ))}
              </div>
            </div>
